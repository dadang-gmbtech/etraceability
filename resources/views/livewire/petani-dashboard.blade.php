<div class="min-h-screen bg-gray-50">
    ...
    <header class="bg-amber-600 text-white shadow">
        <div class="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
            <div>
                <h1 class="text-lg font-bold leading-tight">e-Traceability Gula Kelapa</h1>
                <p class="text-amber-100 text-sm">
                    Portal Petani
                    @if($petani) — {{ $petani->nama }}
                        <span class="opacity-70 text-xs">({{ $petani->kode_petani }})</span>
                    @endif
                </p>
            </div>
            <form method="POST" action="{{ route('portal.logout') }}" class="shrink-0">
                @csrf
                <button type="submit" class="flex items-center gap-1.5 text-sm text-amber-100 hover:text-white transition">
